    ...
    >
      <Menu.Items className="absolute right-0 mt-2 w-56 origin-top-right divide-y divide-gray-100 rounded-md bg-white shadow-lg ring-1 ring-black/5 focus:outline-none">
        <div className="px-1 py-1 ">
          <Menu.Item >
            <p className="mx-auto items-center px-2 py-2 text-sm">Hola José Carlos!</p>
          </Menu.Item>
        </div>
          ...
          <Menu.Item disabled={onSite}>
            {({ active }) => (
              <Link
                href="/profile"
              className={`${
                  (active) ? 'bg-primary/75 text-white' : onSite ? "bg-primary text-white" : 'text-gray-900'
        ...
        <div className="px-1 py-1" onClick={handleSignOut}>          <Menu.Item>
            {({ active }) => (
              <button
                className={`${
